package com.agu_token.api.core
...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { AguTokenInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw AguTokenInvalidDataException("`$name` is not set")
            is JsonNull -> throw AguTokenInvalidDataException("`$name` is null")
            else -> throw AguTokenInvalidDataException("`$name` is invalid, received $this")
        }
